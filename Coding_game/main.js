...
window.alert(beginningScenerios[randomNumber(beginningScenerios.length - 1)]);
...
var character = {
    ...
    characterClass: window.prompt("These times test the strengths for our character. What were you before the war? (Choose from: soldier, doctor or politician)").toLowerCase()
};
...
if (choice === "sneak" || choice === "sneak by the zombie") {
    ...
    if (character.stealth < 5) {
        window.alert("You try to slip by the zombie, but you accidentally stumble over a can of Beanie Weenies, alerting the zombie of your existence. It slowly turns it heads and shuffles towars you.");
        if (character.strength === 5) {
